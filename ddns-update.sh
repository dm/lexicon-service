...
for i in $(seq 0 $(($(jq length "$CONFIG") - 1))); do
  ...
  if lexicon "$provider" update "$zone" "$type" --name "$name" --content "$current_ip" > /dev/null 2>&1; then
    echo "[ddns] Updated $type $name -> $current_ip"
    echo "$current_ip" > "$last_ip_file"
  elif lexicon "$provider" create "$zone" "$type" --name "$name" --content "$current_ip" > /dev/null 2>&1; then
    echo "[ddns] Created $type $name -> $current_ip"
    echo "$current_ip" > "$last_ip_file"
  else
    echo "[ddns] FAILED to set $type $name -> $current_ip (zone: $zone, provider: $provider)"
  fi
done
